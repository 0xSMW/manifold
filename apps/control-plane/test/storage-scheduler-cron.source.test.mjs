...
test("storage Cron routes are Node-only, bearer-authenticated, bounded, and count-only on worker errors", async () => {
  ...
  assert.match(auth, /expected\.trim\(\) !== expected/);
  ...
  assert.match(measure, /measure: runDirectStorageMeasurement/);
  assert.match(measure, /storage_scheduler_due_workspaces/);
  assert.match(measure, /catch \{/);
  assert.doesNotMatch(measure, /error\.message/);
  assert.match(measure, /manifold\.storage\.measure\.failed\.v1/);
  assert.match(measure, /stage: "discovery"/);
  assert.match(measure, /console\.error\(JSON\.stringify\(signal\)\)/);
  assert.match(measure, /storageMeasurementPassStatus\(result\)/);
  assert.match(drain, /compact: \(jobId, workspaceId\) => runDirectCompaction\(jobId, workspaceId/);
  ...
  assert.match(drain, /manifold\.storage\.drain\.failed\.v1/);
  assert.match(drain, /manifold\.storage\.drain\.blocked\.v1/);
  assert.match(drain, /blockerCode: result\.blocker\.code/);
  assert.doesNotMatch(drain, /error\.message/);
  assert.match(drain, /storageDrainPassStatus\(result\)/);
