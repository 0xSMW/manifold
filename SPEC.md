...
- **Round 4 (2026-07-20) — first real vertical slice built (multi-agent: opus×3 + sonnet).** Beyond spec text, built and verified working code: `@manifold/domain` (Money integer-µ$ + banker's rounding + 7-term `computeCost`, and all 8 §5.4 state machines; 32/32 tests); `@manifold/database` (full §6 schema, 47 tables, **applied to a real Postgres 16 container** — B1 composite-PK partitions, immutability triggers, and fail-closed RLS all verified); `packages/ports` + `gateway-core` + `apps/gateway` (runtime-agnostic passthrough gateway: profile-from-host, HMAC key auth, SSRF + header allowlist, flat-memory streaming, async Crypto port M10; 7/7 tests incl. a 256 MB flat-memory stream). **Proved end-to-end:** a single 1-token Haiku call routed *through the gateway* returned `output_tokens=1` and logged an observation. Full-workspace `tsc -b` green; committed. In progress: `@manifold/config` (DB → signed snapshot → gateway loop). Still unbuilt: control-plane `/api/v1`, observation reduce/projections, budget reservation runtime, storage-bounded jobs, console UI, and structural A3/A4/A8.
- **Round 5 (2026-07-20) — config/snapshot engine + loop closed.** `@manifold/config` built and verified on real Postgres 16: `buildSnapshot(db)` → canonical `sha256` content-hash → ed25519 sign/verify (ADR-0024) → **gateway-core loads the built snapshot and routes a request (200)** → `planApply`/`apply` writes exactly one active `gateway_config_revision` + `config_operation`, idempotent re-apply is a no-op, `keyOnlyPublish` (H7) works. The control-plane → DB → signed snapshot → gateway loop now exists. **Integration seams to reconcile (parallel-build debt):** (1) gateway-core verifies no signature — `config.verifySnapshot` is the §7.3 impl a real loader/adapter must call; wire it into the snapshot load path. (2) gateway-core keys routes by `${profileId}:${path}`, not §7.2's `${profile}:${kind}:${public_name}` — reconcile the route-key contract (update gateway-core to the §7.2 key, or amend §7.2). (3) `ports.Snapshot` omits the §7.1 `offerings`/`policies` sections and `SnapshotStore.publish/pointer` — promote config's superset types into `ports`. These are tracked, not silently accepted.
